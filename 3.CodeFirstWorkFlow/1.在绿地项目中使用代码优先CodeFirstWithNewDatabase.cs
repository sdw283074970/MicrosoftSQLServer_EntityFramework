...
//A: 代码优先指数据库随代码变动，本身不对数据库做直接操作。实现这个机制的核心在PackageManager(包管理器)，在PM中使用命令对数据库更新。
  //对于绿地项目，即新项目，在满足所有前提步骤后(前提步骤即一个项目自始至终只用执行一次的步骤)，数据库同步机制如下：
  
  //前提步骤1：安装EntityFramework。在PM中安装EntityFramework；
  //前提步骤2：建立域。使用C#建立一个域(Domain)代表我们控制的数据库实体范围，并以集合以及类、字段的形式建立数据库中的表/列；
  //前提步骤3：添加数据库连接。在App.Config中声明该解决方案连接的connectionStrings；
  //前提步骤4：启用迁移。在PM中启用Migration(迁移)，在项目中建立Migration文件夹并初始化迁移；
  //前提步骤5：同步迁移。在PM使用命令将初始化的迁移与数据库同步一次。
...
//Q: 如何将刚才创建的InitialModel同步至数据库？
//A: 打开PM，使用命令update-database即可完成同步。

//至此绿地项目的代码优先工作流初始化完成。

//暂时想到这么多，最后更新2017/12/19
